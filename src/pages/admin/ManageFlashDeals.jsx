...
export default function ManageFlashDeals() {
    const [deal, setDeal] = useState({ title: "Flash Deals", productIds: [], active: true });
    ...
    const fetchDeal = async () => {
        try {
            setLoading(true);
            const { data } = await axios.get(`${backendUrl}/flash-deals`);
            if (data.success) {
                const d = data.deal;
                setDeal({
                    title: d.title || "Flash Deals",
            ...
            console.error("Error loading flash deal:", err);
            toast.error("Failed to load Flash Deals config");
        } finally {
            setLoading(false);
        }
    };
    ...
    const addProduct = (product) => {
        if (selectedProducts.length >= 20) {
            toast.error("Maximum 20 products allowed for Flash Deals");
            return;
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const { data } = await axios.put(`${backendUrl}/flash-deals`, payload);
            if (data.success) {
                toast.success("Flash Deals saved successfully! 🔥");
            } else {
                toast.error("Failed to save");
            }
        } catch (err) {
            console.error(err);
            toast.error("Failed to save Flash Deals");
        } finally {
            setSaving(false);
        }
    };
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-3">
                <div className="p-2 bg-rose-100 rounded-xl">
                    <LayoutGrid className="text-rose-600 h-6 w-6" />
                </div>
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Flash Deals</h1>
                    <p className="text-sm text-slate-500">Show a 2-row product grid below the Super Category icons (no countdown)</p>
                </div>
            </div>

            {/* Config Card */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
                <h2 className="text-base font-semibold text-slate-800">Section Settings</h2>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${selectedProducts.length >= 20 ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-600"}`}>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        disabled={selectedProducts.length >= 20}
                    />
                ...
                {selectedProducts.length === 0 ? (
                    <div className="py-10 text-center text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
                        <LayoutGrid className="h-8 w-8 mx-auto mb-2 text-slate-300" />
                        <p className="text-sm">No products selected yet. Search and add up to 20.</p>
                    </div>
                ) : (
            ...
            <div className="flex justify-end">
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex items-center gap-2 px-8 py-3 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white font-bold rounded-xl transition-colors shadow-md"
                >
                    {saving ? (
                        <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> Saving...</>
                    ) : (
                        <><LayoutGrid className="h-4 w-4" /> Save Flash Deals</>
                    )}
                </button>
            </div>
        </div>
